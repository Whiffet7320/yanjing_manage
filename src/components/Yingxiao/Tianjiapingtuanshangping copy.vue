    ...
    >
      <div class="myForm">
        <el-form ref="form" :model="addPostageForm" label-width="130px">
          ...
          <el-form-item label="配送区域及运费：">
            <div class="mySkuaddPostageTable">
              <vxe-table border align="center" :data="addPostageData1">
                ...
                <vxe-table-column field="volume" width="170" title="操作">
                  <template #default="{ rowIndex }">
                    <el-button @click="plszTabSku(rowIndex)" size="small" type="text">删除</el-button>
                  </template>
                </vxe-table-column>
              </vxe-table>
            </div>
          ...
          <el-form-item>
            <el-button @click="peisongDialogVisible = true" size="small" type="primary">
              <span style="font-size: 12px">+</span>
              单独添加配送区域
            </el-button>
          </el-form-item>
      ...
      <div class="peisongBox">
        ...
        <div class="nav2">
          ...
          >
            ...
            >
              <el-checkbox-group
                v-model="peisongCheckedCities2"
                @change="peisongHandleCheckedCitiesChange2"
              >
                <!-- 市 -->
                <el-checkbox
                  class="shi"
              ...
              <!-- 省 -->
              <el-checkbox @change="peisongChange" slot="reference" :label="city">{{city.shengVal}}</el-checkbox>
            </el-popover>
          </el-checkbox-group>
        </div>
        <el-button
  ...
  methods: {
    ...
    addSheng() {
      ...
      this.peisongCheckedCities.forEach(ele => {
        ...
        //   // region: [{ name: "默认全国", children: [] }],
        //   region: [{ name: ele.shengVal, children: [] }],
        // })
      });
      console.log(regionName)
    },
    // 运费模板提交
    addShengshiquOnSubmit() {
      console.log();
    },
    // 选择可配送区域
    peisongHandleClose() {
    ...
    peisongHandleCheckedCitiesChange2() {
      console.log(this.peisongCheckedCities2);
      this.peisongCheckedCities.forEach(ele => {
        const myFilter = ele.children.filter(
          item => this.peisongCheckedCities2.indexOf(item) > -1
        );
        ele.myChildren = myFilter;
      });
    ...
    // 批量设置sku
    plszTabSku() {
      console.log(this.skuTableData1[0]);
      this.skuTableData.forEach(ele => {
