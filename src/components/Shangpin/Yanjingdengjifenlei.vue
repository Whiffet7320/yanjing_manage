<template>
  <div class="index">
    <div class="nav1">
      <div class="tit1">眼镜等级管理</div>
    </div>
    <div class="nav2">
      <div class="tit1">
        <el-button @click="toAddShop" size="small" type="primary" icon="el-icon-plus">添加眼镜等级</el-button>
      </div>
      <div class="myTable">
        <vxe-table :data="tableData">
          <vxe-table-column field="id" title="ID"></vxe-table-column>
          <vxe-table-column field="level_name" title="眼镜等级名称"></vxe-table-column>
    ...
    >
      <div class="myAddForm">
        <el-form :model="addForm" ref="addForm" label-width="140px" class="demo-addForm">
          <el-row>
            <el-col :span="20">
              <el-form-item label="眼镜等级名称">
                <el-input size="small" placeholder="请输入眼镜等级名称" v-model="addForm.level_name"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <!-- <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.productlevel({
    ...
    async toDelShop(row) {
      console.log(row);
      const res = await this.$api.deleteproductlevel(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      if (this.isAdd) {
        const res = await this.$api.addproductlevel({
        ...
        const res = await this.$api.updateproductlevel(
          {
            level_name: this.addForm.level_name,
          },
          this.id
