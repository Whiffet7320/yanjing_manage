<template>
  <div class="index">
    <div class="nav1">
      <div class="tit1">标签管理</div>
    </div>
    <div class="nav2">
      <div class="tit1">
        <el-button @click="toAddShop" size="small" type="primary" icon="el-icon-plus">添加标签</el-button>
      </div>
      <div class="myTable">
        <vxe-table :data="tableData">
          <vxe-table-column field="id" title="ID"></vxe-table-column>
          <vxe-table-column field="tags_name" title="标签名称"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="标签名称">
                <el-input size="small" placeholder="请输入标签名称" v-model="addForm.tags_name"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    ...
    async toDelShop(row) {
      console.log(row);
      const res = await this.$api.deleteTags(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      if (this.isAdd) {
        const res = await this.$api.addTags({
          tags_name: this.addForm.tags_name,
        });
        if (res.code == 200) {
          this.$message({
            message: res.message,
        ...
        const res = await this.$api.updateTags(
          {
            tags_name: this.addForm.tags_name,
          },
          this.id
