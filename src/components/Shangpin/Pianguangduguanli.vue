<template>
  <div class="index">
    <div class="nav1">
      <div class="tit1">眼镜偏光率管理</div>
    </div>
    <div class="nav2">
      <div class="tit1">
        <el-button @click="toAddShop" size="small" type="primary" icon="el-icon-plus">添加眼镜偏光率</el-button>
      </div>
      <div class="myTable">
        <vxe-table :data="tableData">
          <vxe-table-column field="id" title="ID"></vxe-table-column>
          <vxe-table-column field="typeinfo.type_name" title="分类名称"></vxe-table-column>
          <vxe-table-column field="basic_index" title="偏光率"></vxe-table-column>
    ...
    >
      <div class="myAddForm">
        <el-form :model="addForm" ref="addForm" label-width="100px" class="demo-addForm">
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="偏光率">
                <el-input size="small" placeholder="请输入偏光率" v-model="addForm.basic_index"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="价格">
                <el-input size="small" placeholder="请输入价格" v-model="addForm.price"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="描述">
                <el-input type="textarea" size="small" placeholder="请输入描述" v-model="addForm.desc"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.basicindex({
    ...
    async toDelShop(row) {
      console.log(row);
      const res = await this.$api.deleteBasicindex(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      if (this.isAdd) {
        const res = await this.$api.addBasicindex({
          product_type_id: this.addForm.product_type_id,
          basic_index: this.addForm.basic_index,
        ...
        const res = await this.$api.updateBasicindex(
          {
            product_type_id: this.addForm.product_type_id,
            basic_index: this.addForm.basic_index,
            price: this.addForm.price,
