    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="color_name" title="颜色名称"></vxe-table-column>
          <vxe-table-column field="color_img" title="颜色图片">
            <template slot-scope="scope">
              <el-image :src="scope.row.color_img" fit="fill" style="width: 40px; height: 40px">
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="颜色名称">
                <el-input size="small" placeholder="请输入颜色名称" v-model="addForm.color_name"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.color({
    ...
    async toDelShop(row) {
      console.log(row);
      const res = await this.$api.deleteColor(row.id);
      if (res.code == 200) {
        this.$message({
          message: res.message,
          type: "success"
    ...
    async AddOnSubmit() {
      if (this.isAdd) {
        const res = await this.$api.addColor({
          product_type_id: this.addForm.product_type_id,
          color_name: this.addForm.color_name,
          color_img: this.addForm.color_img
        });
        if (res.code == 200) {
          this.$message({
            message: res.message,
        ...
        const res = await this.$api.updateColor(
          {
            product_type_id: this.addForm.product_type_id,
            color_name: this.addForm.color_name,
            color_img: this.addForm.color_img
